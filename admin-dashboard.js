...
async function _loadAdminOverview() {
  ...
  try {
    const [
      ...
      sb.from('profiles').select('*',{count:'exact',head:true}).eq('role','merchant'),
    ...
    const urgent = (pendingDeals || 0) + (editReqs || 0) + (delReqs || 0);
    ...
    content.innerHTML = `
      ...
      <div class="dash-stats">
        ...
        ${_admStat('🏪', 'تجار مسجلين',   merchants    || 0, '#eff6ff', 'merchants')}
        ${_admStat('✏️', 'طلبات تعديل',   editReqs     || 0, '#faf5ff', 'edit-requests')}
        ${_admStat('🗑️', 'طلبات حذف',     delReqs      || 0, '#fef2f2', 'delete-requests')}
      ...
      <div class="adm-quick-grid">
        ...
        <div class="adm-quick-card" onclick="switchAdminSection('delete-requests')">
          <div class="adm-quick-ico">🗑️</div>
          <div class="adm-quick-label">طلبات الحذف</div>
          <div class="adm-quick-count">${delReqs || 0} معلق</div>
        </div>
