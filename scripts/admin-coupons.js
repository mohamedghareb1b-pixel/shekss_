...
async function deleteAdminCoupon(couponId) {
  ...
  _acpCouponsCache = _acpCouponsCache.filter(c => c.id !== couponId);
  toast('🗑️ تم حذف الكوبون');
}

// ===== EDIT COUPON =====
function acpOpenEditModal(couponId) {
  const c = _acpCouponsCache.find(x => x.id === couponId);
  if (!c) return;

  const overlay = document.createElement('div');
  overlay.id = 'acp-edit-overlay';
  overlay.className = 'aad-modal-overlay';
  overlay.innerHTML = `
    <div class="aad-modal">
      <div class="aad-modal-head">
        <h3>✏️ تعديل الكوبون</h3>
        <button class="aad-modal-close" onclick="acpCloseEditModal()">✕</button>
      </div>
      <div class="aad-modal-body">
        <div class="form-msg error" id="acp-edit-err"></div>

        <div class="form-group">
          <label class="form-label">اسم الموقع / المتجر</label>
          <input class="form-input" type="text" id="acp-edit-site" value="${_acpEsc(c.site_name || '')}">
        </div>
        <div class="form-group">
          <label class="form-label">كود الكوبون</label>
          <input class="form-input" type="text" id="acp-edit-code" dir="ltr"
                 style="text-transform:uppercase;" value="${_acpEsc(c.code || '')}">
        </div>
        <div class="price-row">
          <div class="form-group">
            <label class="form-label">نوع الخصم</label>
            <select class="form-input" id="acp-edit-type">
              <option value="percent" ${c.type === 'percent' ? 'selected' : ''}>نسبة مئوية %</option>
              <option value="fixed" ${c.type === 'fixed' ? 'selected' : ''}>مبلغ ثابت جنيه</option>
              <option value="free_shipping" ${c.type === 'free_shipping' ? 'selected' : ''}>شحن مجاني</option>
              <option value="other" ${c.type === 'other' ? 'selected' : ''}>أخرى</option>
            </select>
          </div>
          <div class="form-group">
            <label class="form-label">قيمة الخصم</label>
            <input class="form-input" type="number" id="acp-edit-discount" value="${c.discount || ''}" min="0" step="0.01">
          </div>
        </div>
        <div class="form-group">
          <label class="form-label">رابط الشعار</label>
          <input class="form-input" type="url" id="acp-edit-logo" dir="ltr" value="${_acpEsc(c.logo || '')}">
        </div>
        <div class="form-group">
          <label class="form-label">تاريخ الانتهاء</label>
          <input class="form-input" type="date" id="acp-edit-expires"
                 value="${c.expires_at ? c.expires_at.split('T')[0] : ''}">
        </div>
      </div>
      <div class="aad-modal-foot">
        <button class="dash-btn dash-btn-ghost" onclick="acpCloseEditModal()">إلغاء</button>
        <button class="dash-btn dash-btn-primary" id="acp-edit-save-btn"
                onclick="acpSaveEdit('${c.id}')">💾 حفظ</button>
      </div>
    </div>`;
...
function acpCloseEditModal() {
  document.getElementById('acp-edit-overlay')?.remove();
}

async function acpSaveEdit(couponId) {
  const site     = document.getElementById('acp-edit-site')?.value.trim();
  const code     = document.getElementById('acp-edit-code')?.value.trim().toUpperCase();
  const type     = document.getElementById('acp-edit-type')?.value;
  const discount = parseFloat(document.getElementById('acp-edit-discount')?.value) || null;
  const logo     = document.getElementById('acp-edit-logo')?.value.trim() || null;
  const expires  = document.getElementById('acp-edit-expires')?.value || null;

  const errEl = document.getElementById('acp-edit-err');
  const btn   = document.getElementById('acp-edit-save-btn');
  ...
  const updateData = {
    site_name: site, code, type, discount, logo,
    expires_at: expires ? new Date(expires).toISOString() : null,
  };

  const { error } = await sb.from('coupons').update(updateData).eq('id', couponId);
  ...
  const idx = _acpCouponsCache.findIndex(c => c.id === couponId);
  if (idx !== -1) {
    Object.assign(_acpCouponsCache[idx], updateData);
    const cardEl = document.getElementById(`acp-card-${couponId}`);
    if (cardEl) cardEl.outerHTML = _couponCard(_acpCouponsCache[idx]);
  }
}
