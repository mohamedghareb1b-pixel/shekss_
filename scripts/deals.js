...
function dealCard(d, mode, isExpired = false) {
  // Support both column naming conventions: image/image2 and main_image/second_image
  const primaryImg   = d.image || d.main_image || null;
  const secondaryImg = d.image2 || d.second_image || null;
  ...
  if (secondaryImg && primaryImg) {
    ...
    imgHtml = `
      <div class="dimg-slider" id="${uid}"
           ontouchstart="dSlideTouchStart(event,'${uid}')"
           ontouchend="dSlideTouchEnd(event,'${uid}')">
        <img src="${primaryImg}" alt="${d.name}" class="dslide active" onerror="this.style.display='none'">
        <img src="${secondaryImg}" alt="${d.name}" class="dslide" onerror="this.style.display='none'">
        <button class="dslide-arrow dslide-arrow-prev" onclick="event.stopPropagation();slideImgStep('${uid}',-1)" aria-label="الصورة السابقة">‹</button>
        <button class="dslide-arrow dslide-arrow-next" onclick="event.stopPropagation();slideImgStep('${uid}',1)" aria-label="الصورة التالية">›</button>
        <button class="dslide-dot dot-0 active" onclick="event.stopPropagation();slideImg('${uid}',0)"></button>
        <button class="dslide-dot dot-1" onclick="event.stopPropagation();slideImg('${uid}',1)"></button>
      </div>`;
  } else if (primaryImg) {
    imgHtml = `<img src="${primaryImg}" alt="${d.name}" onerror="this.parentElement.innerHTML='<div class=dimg-placeholder>🏷️</div>'">`;
  } else {
    imgHtml = '<div class="dimg-placeholder">🏷️</div>';
  }

  const hot     = (!isExpired && d.status === 'hot') ? '<span class="badge-hot">ساخن 🔥</span>' : '';
  const offline = mode === 'offline' ? '<span class="badge-offline">📍 عروض المنطقة</span>' : '';
  ...
  // Discount badge text — handles: fixed discount/discount_percent,
  // discount range (from-to), or none at all (badge hidden)
  let discountBadge = '';
  if (d.discount_from && d.discount_to) {
    discountBadge = `${d.discount_from}-${d.discount_to}%`;
  } else {
    const discVal = d.discount || d.discount_percent || null;
    if (discVal) discountBadge = `${discVal}%`;
  }

  // Price HTML — support both `price`/`discount` and `old_price`/`new_price` columns
  const origPrice = parseFloat(d.price || d.old_price || 0);
  const newPrice  = d.new_price != null ? parseFloat(d.new_price) : null;

  let priceHtml = '';
  if (origPrice && newPrice != null && newPrice < origPrice) {
    priceHtml = `<div class="dprice-wrap">
      <span class="dprice-old">${origPrice.toLocaleString('ar-EG')} ج</span>
      <span class="dprice-new">${newPrice.toLocaleString('ar-EG')} ج</span>
      <span class="dprice-save">وفّر ${(origPrice - newPrice).toLocaleString('ar-EG')} ج</span>
    </div>`;
  } else if (origPrice && d.discount) {
    const disc  = parseFloat(d.discount);
    const after = Math.round(origPrice * (1 - disc / 100));
    priceHtml = `<div class="dprice-wrap">
      <span class="dprice-old">${origPrice.toLocaleString('ar-EG')} ج</span>
      <span class="dprice-new">${after.toLocaleString('ar-EG')} ج</span>
      <span class="dprice-save">وفّر ${(origPrice - after).toLocaleString('ar-EG')} ج</span>
    </div>`;
  } else if (origPrice) {
    priceHtml = `<div class="dprice-wrap"><span class="dprice-new">${origPrice.toLocaleString('ar-EG')} ج</span></div>`;
  }
  ...
  return `<div class="deal-card" data-cat="${d.category || ''}" style="${cardStyle}">
    <div class="dimg" style="position:relative;cursor:pointer;" onclick="openDealPage('${d.id}')">${imgHtml}${discountBadge ? `<span class="badge-disc">${discountBadge}</span>` : ''}${hot}${offline}${expiredOverlay}</div>
    <div class="dbody">
      <div class="dcat">${catLabel}${subcatLabel}</div>
      ${storeName}
      <div class="dname" style="cursor:pointer;" onclick="openDealPage('${d.id}')">${d.name}</div>
      <div class="ddesc" style="cursor:pointer;" onclick="openDealPage('${d.id}')">${d.description || ''}</div>
...
function buildHomeCats() {
  ...
  document.getElementById('home-cats-title').textContent = homeMode === 'online' ? '📂 أقسام أونلاين' : '📍 أقسام عروض المنطقة';
...
function openDealPage(dealId) {
  history.pushState({ page: 'deal', id: dealId }, '', `/?p=deal&id=${dealId}`);
  showPage('deal');
}
