    ...
    <div class="dp-wrap">
      ...
      <div class="dp-info">
        ...
        <div class="dp-desc-block">
          <p class="dp-section-body" id="dp-desc-text" data-full="${_dpEsc(desc)}" data-short="${_dpEsc(descShort)}">${_dpEsc(descTruncated ? descShort : desc)}</p>
          ${descTruncated ? `
          <button class="dp-show-more-btn" id="dp-show-more-btn" data-expanded="false"
                  onclick="dpToggleDescription()">
            اظهر المزيد ▾
          </button>` : ''}
        </div>` : ''}
...
function dpToggleDescription() {
  const textEl = document.getElementById('dp-desc-text');
  const btnEl  = document.getElementById('dp-show-more-btn');
  if (!textEl || !btnEl) return;

  // data-full / data-short are HTML-escaped values set when the page rendered.
  // Reading via .dataset auto-decodes HTML entities back to plain text.
