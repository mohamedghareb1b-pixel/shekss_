...
async function renderAdminAllDeals() {
  ...
  content.innerHTML = `
    <div class="form-group" style="margin-bottom:14px;">
      <input class="form-input" type="text" id="aad-search-input"
             placeholder="🔍 ابحث بالاسم، المتجر، أو التصنيف..."
             oninput="aadApplyFilters()" autocomplete="off">
    </div>
...
// ===== FILTER STATE =====
let _aadActiveTypeFilter = 'all';

// ===== TYPE FILTER (chips) =====
function aadFilterDeals(type) {
  _aadActiveTypeFilter = type;
...
function aadApplyFilters() {
  const query = (document.getElementById('aad-search-input')?.value || '').trim().toLowerCase();
  const list  = document.getElementById('aad-all-list');
  if (!list) return;

  let filtered = _aadAllDealsCache;

  // Apply type filter (admin-owned / merchant-owned / all)
  if (_aadActiveTypeFilter === 'admin')    filtered = filtered.filter(d => !d.merchant_id);
  if (_aadActiveTypeFilter === 'merchant') filtered = filtered.filter(d => d.merchant_id);

  // Apply search text — matches title/name, company, or category
  if (query) {
    filtered = filtered.filter(d => {
      const title    = (d.title || d.name || '').toLowerCase();
      const company  = (d.company_name || '').toLowerCase();
      const category = (d.category || '').toLowerCase();
      const catLabel = (CAT_LABELS[d.category] || '').toLowerCase();
  ...
  list.innerHTML = filtered.map(d => _allDealCard(d)).join('')
    || `<div style="text-align:center;padding:32px;color:var(--gray-400);">
          ${query ? `لا توجد نتائج لـ "${_aadEsc(query)}"` : 'لا توجد عروض في هذا القسم'}
        </div>`;
}
